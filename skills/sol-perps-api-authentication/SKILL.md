---
name: sol-perps-api-authentication
description: "Handle Sol Perps Dex API seeds securely, construct exact Ed25519 request signatures, and diagnose authentication with read-only checks."
---

# Sol Perps Dex API authentication

Use for API key setup or signature failures. This guide supplies no signer or execution tool. Broker: `sol_perps_dex`. Website: [solperpsbot.io](https://solperpsbot.io); [docs](https://docs.solperpsbot.io); [key UI](https://solperpsbot.io/portfolio/api-key).
...
Never rebrand these headers. Broker `sol_perps_dex` belongs in supported account/application configuration, not an invented order field.

## Verify with a read, not a trade

With an existing authorized secure execution tool, begin with `GET /v1/client/info`. Inspect HTTP status and JSON `success`; do not treat every HTTP 200 as success. Return only a sanitized readiness result. Follow [account reads](../sol-perps-account-data/SKILL.md) for holdings, positions, and orders.
...
Re-signing a read after correcting a diagnosed error can be appropriate within rate limits. Re-signing a timed-out order is **not** a harmless auth retry: an ambiguous write must be reconciled by ID under [trading orders](../sol-perps-trading-orders/SKILL.md), never blindly resubmitted.

## Navigation and evidence

[Onboarding](../sol-perps-onboarding/SKILL.md) · [Positions/risk](../sol-perps-positions-risk/SKILL.md) · [Index](../../README.md) · [Sources](../../SOURCES.md)
