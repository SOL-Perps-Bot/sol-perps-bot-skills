---
name: sol-perps-positions-risk
description: "Inspect Sol Perps Dex positions and collateral, manage explicitly approved symbol leverage, and plan separately authorized reduce-only closes."
---

# Sol Perps Dex positions and risk

Use for exposure/risk reads or a precisely authorized change. No execution tool is included. Broker `sol_perps_dex`; [website](https://solperpsbot.io), [docs](https://docs.solperpsbot.io), [key UI](https://solperpsbot.io/portfolio/api-key).

## Read first

Default to read-only/testnet `https://testnet-api.orderly.org`; mainnet is `https://api.orderly.org`. Use an authorized secure signer and [authentication](../sol-perps-api-authentication/SKILL.md), never raw secrets in chat/logs. Broker is account context, not an invented order parameter.

1. Read `GET /v1/positions`, account information, holdings, and relevant pending orders using [account data](../sol-perps-account-data/SKILL.md).
...
3. Require `reduce_only: true` on the separately approved `POST /v1/order`. Preserve margin mode and enforce pre-send caps via [trading orders](../sol-perps-trading-orders/SKILL.md). Never remove reduce-only to bypass rejection.
