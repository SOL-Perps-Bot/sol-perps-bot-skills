---
name: sol-perps-onboarding
description: "Set up Sol Perps Dex context safely and select guides for authentication, account reads, scoped orders, and position risk."
---

# Sol Perps Dex onboarding
...
2. Confirm broker `sol_perps_dex`, the intended account, and environment. Mainnet REST is `https://api.orderly.org`; testnet REST is `https://testnet-api.orderly.org`. Do not silently switch environments or substitute another broker if setup fails.
3. Direct the user to [solperpsbot.io](https://solperpsbot.io), [product docs](https://docs.solperpsbot.io), and [API key management at /portfolio/api-key](https://solperpsbot.io/portfolio/api-key). The user performs wallet approvals in the trusted UI. Do not request their wallet seed or API secret.
...
6. Follow [authentication](../sol-perps-api-authentication/SKILL.md) and then [account data](../sol-perps-account-data/SKILL.md). Stop at a sanitized read summary unless more was explicitly requested.

## Broker and tool boundaries

`sol_perps_dex` identifies the account's broker relationship in the supported registration/lookup and application configuration flows. It is **not an arbitrary order parameter**. Do not create a new broker, pay a registration fee, or inject broker/referral tags into orders as part of onboarding.
...
- [Trading orders](../sol-perps-trading-orders/SKILL.md): one authorized order and lifecycle reconciliation.
- [Positions and risk](../sol-perps-positions-risk/SKILL.md): risk reads and separately approved leverage/reductions.
